...
const FilesList = ({ request, setExpand }: Props) => {
    ...
    return <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        <button onClick={() => { downloadAndZip(request.contentFileUrls?.map((x: any, i: number) => x.fileUrl)) }}>Download all</button>
        <div className=" text-m" style={{ display: "flex", flexDirection: "column" }}>{request.contentFileUrls?.map((x: any, i: number) => <a key={x.fileUrl} href={x.fileUrl}>{i + 1}: {x.fileName}</a>)}</div>
    </div>
}
...
export const ExpandedCell = ({ request, setExpand }: Props) => {
    ...
    return <div className="bg-gray-50 hover:bg-gray-100 rounded-2xl shadow-md p-10 space-y-2 w-full">
        ...
        <button onClick={e => { e.preventDefault(); setExpand(false) }}>Close</button>
    </div>
}
