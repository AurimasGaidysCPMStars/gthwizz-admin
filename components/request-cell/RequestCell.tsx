...
export const RequestCell = ({ id, data }: Props) => {
    ...
    return <div className="bg-gray-50 hover:bg-gray-100 rounded-2xl shadow-md p-10 space-y-2 w-full">
        ...
        <div className=" text-m">{request.fileUrls.map(x => <a key={x.fileUrl} href={x.fileUrl}>{x.fileName}</a>)}</div>
        ...
        <div className=" text-m">{request.contentFileUrls?.map(x => <a key={x.fileUrl} href={x.fileUrl}>{x.fileName}</a>)}</div>
